// Pre-baked POC rosters (design/rosters.md). Two ~78-OVR teams with distinct
// identities so headless sims exercise the full contest spectrum:
//   HOME "Anvil" — balanced, strong interior OL, sure tackling.
//   AWAY "Bolt"  — boom-bust: elite EDGE rush + elite RB, weaker OL edges & CB2.
// Ratings are sparse 0..99; anything unlisted defaults to 70. (AGG is a small
// signed bias, BSH_edge is an ability flag — both non-rating, read explicitly.)

export type PlayerRatings = Record<string, number | boolean>;
export type Roster = Record<string, PlayerRatings>;
...
export const ROSTERS: Record<"home" | "away", Roster> = {
  home: HOME_Anvil,
  away: AWAY_Bolt,
};

/** read a rating (0..99); missing/non-numeric falls back to the 70 default */
export function rate(r: PlayerRatings | undefined, key: string): number {
  const v = r?.[key];
  return typeof v === "number" ? v : DEFAULT_RATING;
}
